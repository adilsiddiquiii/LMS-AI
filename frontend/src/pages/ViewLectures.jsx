    ...
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200">
        <div className="mb-6">
          ...
          <div className="mt-2 flex gap-4 text-sm text-gray-500 font-medium">
            <span>Cetegory :{selectedCourse?.category}</span>
            <span> Level : {selectedCourse?.level}</span>
          </div>
          {/* Video */}
          <div className=" aspect-video bg-black rounded-lg overflow-hidden mb-4 mt-2 border border-gray-300">
      ...
      { createrData &&
        ...
        <div className="flex items-center gap-4">
          <img className="w-14 h-14 rounded-full object-cover" src={createrData?.photoUrl} alt="" />
        </div>
        <div>
          <h2 className="text-base font-medium text-gray-800"
          >{createrData?.name}</h2>
          <p className="text-sm text-gray-600">{createrData?.description}</p>
          <p className="text-sm text-gray-600">{createrData?.email}</p>
        </div>
      </div>}
      </div>
    </div>
